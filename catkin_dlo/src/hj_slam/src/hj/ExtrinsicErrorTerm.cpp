...
int ExtrinsicErrorTerm::runCalibExtrinsicRansac(VecOfPoses odom_poses, VecOfPoses laser_poses, double& x, double& y, double& z, double& yaw, double& pitch, double& roll)
{
    ...
    VecOfPoses temp_odom_poses;
    VecOfPoses temp_laser_poses;
    for(int i = 0; i < laser_poses.size(); i++)
    {
        if(i % 100 == 0)
        {
            temp_laser_poses.push_back(laser_poses[i]);
            temp_odom_poses.push_back(odom_poses[i]);
        }
    }
    laser_poses = temp_laser_poses;
    odom_poses = temp_odom_poses;
