...
void CLaserOdometry2D::PoseUpdate()
{
  ...
  s2s_pose_ = pose_aux_2D;
  //std::cout<<pose_aux_2D.translation()(0)<<" "<<pose_aux_2D.translation()(1)<<" "<<rf2o::getYaw(pose_aux_2D.rotation())<<std::endl;
  ...
  //   std::string kittipath = "/home/zc/kitti_rf2o.txt";
  // std::ofstream out_kitti(kittipath, std::ios::app);
  // T.block<3,3>(0,0) = laser_pose_.rotation().cast<double>();
  // T.block<3,1>(0,3) = laser_pose_.translation().cast<double>();
  // out_kitti << std::fixed << std::setprecision(9)
  //           << T(0, 0) << " " << T(0, 1) << " " << T(0, 2) << " " << T(0, 3) << " "
  //           << T(1, 0) << " " << T(1, 1) << " " << T(1, 2) << " " << T(1, 3) << " "
  //           << T(2, 0) << " " << T(2, 1) << " " << T(2, 2) << " " << T(2, 3) << "\n";
  // pcl::transformPointCloud(*cloud_, *cloud_, T.cast<float>());
  // *map_ += *cloud_;
  // pcl::io::savePLYFile("/home/zc/rf2o_map.ply", *map_);
