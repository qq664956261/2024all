...
hj::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle)
{
  ...
  m_iExtendedKalmanFilter_ = ExtendedKalmanFilter();
  m_iMatrixXdEkfQ_ =
  ...
  laser_odometry_ptr_ = std::make_shared<rf2o::CLaserOdometry2D>();
  undistort_ptr_ = std::make_shared<PointUndistort>();
...
void hj::OdomNode::preprocessPoints()
{
  ...
  this->original_scan = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>);
  pcl::copyPointCloud(*this->current_scan, *this->original_scan);
...
void hj::OdomNode::initializeInputTarget()
{
  ...
  this->keyframes_loop_.push_back(std::make_pair(std::make_pair(this->pose, this->rotq), original_scan));
 ...
 void hj::OdomNode::icpCB(const sensor_msgs::LaserScan::ConstPtr &pc)
{
  ...
  // {
  //   laser_scan_ = *pc;
  //   if (rf2o_first_frame_)
  //   {
  //     rf2o_first_frame_ = false;
  ...
  //     laser_odometry_ptr_->init(*pc, initial_robot_pose.pose.pose);
  //   }
  // }
  ...
  {
    double start_time;
    start_time = lidar_time_;
    double end_time;
    double time_increment;
    time_increment = pc->time_increment;
    end_time = start_time + time_increment * pc->ranges.size();
    undistort_ptr_->setLidarTime(start_time, end_time, time_increment);
  }
  ...
  if (this->imu_use_)
  {
    ...
    {
      this->mtx_imu.lock();
      double imu_time = this->imu_buffer.front().stamp;
      this->mtx_imu.unlock();
      if (imu_time - lidar_time_ > 0.1)
      {
        break;
      }
      else
    ...
    if (use_undistort_)
    {
      auto scan_undistort = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>);
      scan_undistort = undistort_ptr_->GetUndistortedPointCloudByIMU(current_scan);
      pcl::copyPointCloud(*scan_undistort, *this->current_scan);
    }
  }
...
// void hj::OdomNode::imuCB(const hj_interface::ImuConstPtr &imu)
{
  ...
  if (!this->imu_calibrated)
  {
    ...
    // if ((imu->custom_time.toSec() - this->first_imu_time) < 0)
    {
...
void hj::OdomNode::getNextPose()
{
  ...
  else
  {
    if (use_icp_)
    {
      if (use_rf2o_)
      {
        ...
        laser_odometry_ptr_->odometryCalculation(laser_scan_);
        // ros::WallDuration m_runtime = ros::WallTime::now() - start;
        // ROS_INFO_COND(true, "[rf2o] execution time (ms): %f",
  ...
  if (use_icp_)
  {
    if (use_rf2o_)
    {
      Eigen::Isometry3d increment = laser_odometry_ptr_->getIncrement();
      T_S2S = increment.matrix().cast<float>();
    }
    else
    {
  ...
  Eigen::Matrix4f fusion_result;
  fusion_result.setIdentity();
  if (use_rf2o_)
  {
    if (!m_iExtendedKalmanFilter_.IsInitialized())
    {
      Eigen::Vector3d t = this->T.block(0, 3, 3, 1).cast<double>();
      Eigen::Matrix3d r = this->T.block(0, 0, 3, 3).cast<double>();
      Eigen::Quaterniond q(r);
      Pose pose_aligned(t, q);
      m_iExtendedKalmanFilter_.Initialize(pose_aligned,
                                          laser_scan_.header.stamp.toSec());
      m_iExtendedKalmanFilter_.SetQ(m_iMatrixXdEkfQ_);
      m_iExtendedKalmanFilter_.SetR(m_iMatrixXdEkfR_);
      m_iExtendedKalmanFilter_.SetOdomAngle(m_dOdomToLidar_);
    }
    else
    {
      Odometry cur_odom;
      Eigen::Vector3d odom_t = this->T_s2s.block(0, 3, 3, 1).cast<double>();
      Eigen::Matrix3d odom_r = this->T_s2s.block(0, 0, 3, 3).cast<double>();
      Eigen::Quaterniond odom_q(odom_r);
      Pose odom_pose(odom_t, odom_q);
      cur_odom.pose = odom_pose;
      cur_odom.header.stamp = laser_scan_.header.stamp.toSec();
      Eigen::Vector3d lidar_t = this->T.block(0, 3, 3, 1).cast<double>();
      Eigen::Matrix3d lidar_r = this->T.block(0, 0, 3, 3).cast<double>();
      Eigen::Quaterniond lidar_q(lidar_r);
      Pose lidar_pose(lidar_t, lidar_q);
      if (fusionEkf(cur_odom, lidar_pose))
      {
        fusion_result.block(0, 3, 3, 1) = m_iPoseEkf_.position.cast<float>();
        fusion_result.block(0, 0, 3, 3) = m_iPoseEkf_.quaternion.toRotationMatrix().cast<float>();
      }
    }
  }

  std::string kittipath = "/home/zc/kitti_traj.txt";
  std::ofstream out_kitti(kittipath, std::ios::app);
  if (use_rf2o_)
  {
    out_kitti << std::fixed << std::setprecision(9)
              << fusion_result(0, 0) << " " << fusion_result(0, 1) << " " << fusion_result(0, 2) << " " << fusion_result(0, 3) << " "
              << fusion_result(1, 0) << " " << fusion_result(1, 1) << " " << fusion_result(1, 2) << " " << fusion_result(1, 3) << " "
              << fusion_result(2, 0) << " " << fusion_result(2, 1) << " " << fusion_result(2, 2) << " " << fusion_result(2, 3) << "\n";
  }
  else
  {
    out_kitti << std::fixed << std::setprecision(9)
              << this->T(0, 0) << " " << this->T(0, 1) << " " << this->T(0, 2) << " " << this->T(0, 3) << " "
              << this->T(1, 0) << " " << this->T(1, 1) << " " << this->T(1, 2) << " " << this->T(1, 3) << " "
              << this->T(2, 0) << " " << this->T(2, 1) << " " << this->T(2, 2) << " " << this->T(2, 3) << "\n";
  }
  if (is_optimized_)
  {
...
void hj::OdomNode::integrateIMU()
{
  ...
  boost::circular_buffer<IMUData> imu_buffer_undistort;
  imu_buffer_undistort.set_capacity(1000);
  // std::cout << "prev_frame_imu_dt:" << std::to_string(prev_frame_stamp) << std::endl;
  // std::cout << "curr_frame_imu_dt:" << std::to_string(curr_frame_stamp) << std::endl;
  for (const auto &i : this->imu_buffer)
  {
    ...
    if (curr_frame_imu_dt >= -1. && prev_frame_imu_dt <= 0.)
    {
      IMUData imu_data;
      imu_data.gyro.x() = i.ang_vel.x;
      imu_data.gyro.y() = i.ang_vel.y;
      imu_data.gyro.z() = i.ang_vel.z;
      imu_data.acc.x() = i.lin_accel.x;
      imu_data.acc.y() = i.lin_accel.y;
      imu_data.acc.z() = i.lin_accel.z;
      imu_data.timestamp = i.stamp;
      imu_buffer_undistort.push_front(imu_data);
    }
  }
  if (use_undistort_)
  {
    undistort_ptr_->setIMUBuffer(imu_buffer_undistort);
  }
  ...
  // std::cout << "imu_SE3:" << imu_SE3 << std::endl;
  // std::cout << "odom sum_dt:" << sum_dt << std::endl;
}
...
void hj::OdomNode::updateKeyframes()
{
  ...
  //         this->keyframes_timestamps_.push_back(scan_stamp.toSec());
  //         pcl::transformPointCloud(*this->original_scan, *this->original_scan, this->T);
  //         this->keyframes_loop_.push_back(std::make_pair(std::make_pair(this->pose, this->rotq), original_scan));
...
void hj::OdomNode::checkForTimeout(const ros::TimerEvent &)
{
  ...
  ros::Duration timeout_duration(20.0); // 20秒无消息则认为超时
  ...
  while (ros::ok())
  {
    if (start_calib_)
    {
      if (use_fuison_result_)
      {
        ...
        std::cout << "lidar_poses_.size():" << lidar_poses_.size() << std::endl;
        std::cout << "odom_poses_.size():" << odom_poses_.size() << std::endl;
...
void hj::OdomNode::saveResult(const Eigen::Matrix4d &T_odom_lidar)
{
...
void hj::OdomNode::align()
{
  ...
  Eigen::AngleAxisd y(-0.12, Eigen::Vector3d::UnitZ());
  Eigen::Quaterniond q = y * p * r;
  R_ = q.toRotationMatrix();
  t_ = Eigen::Vector3d(-0.15, 0, 0);
...
void hj::OdomNode::runLoopClosure()
{
  while (ros::ok())
  {
    if (loop_detected_)
    {
      ...
      pcl::copyPointCloud(*keyframes_loop_.back().second, *source_cloud);
      pcl::copyPointCloud(*keyframes_loop_[loop_id_].second, *target_cloud);
      pcl::PointCloud<PointType>::Ptr aligned_cloud(new pcl::PointCloud<PointType>);
      Eigen::Matrix4f T = Eigen::Matrix4f::Identity();
      Eigen::Matrix4f T_source = Eigen::Matrix4f::Identity();
      T_source.block<3, 3>(0, 0) = keyframes_loop_[loop_size_ - 1].first.second.toRotationMatrix().cast<float>();
      T_source.block<3, 1>(0, 3) = keyframes_loop_[loop_size_ - 1].first.first;
      Eigen::Matrix4f T_target1 = Eigen::Matrix4f::Identity();
      T_target1.block<3, 3>(0, 0) = keyframes_loop_[loop_id_].first.second.toRotationMatrix().cast<float>();
      T_target1.block<3, 1>(0, 3) = keyframes_loop_[loop_id_].first.first;
      ...
      this->mtx_loop_.lock();
      T_target.block<3, 3>(0, 0) = keyframes_loop_[loop_id_].first.second.toRotationMatrix().cast<double>();
      T_target.block<3, 1>(0, 3) = keyframes_loop_[loop_id_].first.first.cast<double>();
      this->mtx_loop_.unlock();
  ...
  if (ceres_converge)
  {
    ...
    for (int i = 0; i < loop_size_; i++)
    {
      ...
      *map_before += *keyframes_loop_[i].second;
      std::string kittipath = "/home/zc/kitti_traj_opti.txt";
      std::ofstream out_kitti(kittipath, std::ios::app);

      out_kitti << std::fixed << std::setprecision(9)
                << pose_tran(0, 0) << " " << pose_tran(0, 1) << " " << pose_tran(0, 2) << " " << pose_tran(0, 3) << " "
                << pose_tran(1, 0) << " " << pose_tran(1, 1) << " " << pose_tran(1, 2) << " " << pose_tran(1, 3) << " "
                << pose_tran(2, 0) << " " << pose_tran(2, 1) << " " << pose_tran(2, 2) << " " << pose_tran(2, 3) << "\n";
    }
    ...
    std::cout << "T_after1:" << T_after_ << std::endl;
    // T_after_ = pose2.cast<float>();
    std::cout << "T_after2:" << pose2 << std::endl;
    this->mtx_loop_.unlock();
    is_optimized_ = true;
    last_loop_time_ = scan_stamp.toSec();
  }
...
bool hj::OdomNode::fusionEkf(Odometry sys_odom, Pose lidar_pose)
{

  int64_t curodomtime = (int64_t)(sys_odom.header.stamp);

  Pose transFormWorldToBase = Pose(sys_odom.pose.position[0], sys_odom.pose.position[1], 0, 0, 0, sys_odom.pose.yaw);

  // m_iExtendedKalmanFilter_.InputOdomMeasurement(transFormWorldToBase);
  m_iExtendedKalmanFilter_.InputOdomMeasurement(sys_odom);
  m_iExtendedKalmanFilter_.AddLidarMeasurementOld(lidar_pose);

  m_iPoseEkf_ = Pose(m_iExtendedKalmanFilter_.GetX()(0), m_iExtendedKalmanFilter_.GetX()(1), lidar_pose.position.z(), 0.0, 0.0, m_iExtendedKalmanFilter_.GetX()(2));
