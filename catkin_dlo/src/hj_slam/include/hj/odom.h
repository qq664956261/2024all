...
private:
  ...
  std::shared_ptr<rf2o::CLaserOdometry2D> laser_odometry_ptr_;
  bool use_rf2o_{false};
  bool rf2o_first_frame_{true};
  sensor_msgs::LaserScan laser_scan_;
  std::vector<double> vEkfQ_{0.01, 0.0, 0.0, 0.0, 0.01, 0.0, 0.0, 0, 0.01};
  std::vector<double> vEkfR_{0.5, 0.0, 0.0, 0.0, 0.5, 0.0, 0.0, 0.0, 0.5};
  Eigen::MatrixXd m_iMatrixXdEkfQ_;
  Eigen::MatrixXd m_iMatrixXdEkfR_;
  ExtendedKalmanFilter m_iExtendedKalmanFilter_;
  Pose m_iPoseEkf_;
  double m_dOdomToLidar_;

  bool use_undistort_{true};
  std::shared_ptr<PointUndistort> undistort_ptr_;
};
