 ...
 */
// SensorBridge 是 Cartographer ROS 框架中负责将 ROS 的传感器数据（如激光雷达、IMU、GPS 等）转换为 Cartographer SLAM 系统可处理的内部数据格式的关键模块
...
// 将 ROS 的 nav_msgs::Odometry 消息格式转换为 Cartographer 内部的 carto::sensor::OdometryData 格式。
// 在转换过程中，通过 tf_bridge_ 查找必要的坐标变换，将里程计数据从其原始坐标系转换到 Cartographer 的 tracking frame
//sensor_to_tracking sensor到tracking frame的外参 外参在tf_buffer里（urdf里读或者rosbag /tf_static）
...
// 处理 ROS 中的里程计（Odometry）消息，将其转换为 Cartographer 内部的 OdometryData 数据格式，并将转换后的数据传递给轨迹构建器 (trajectory_builder_) 进行处理
...
// 处理 GPS 数据（sensor_msgs::NavSatFix 消息类型），并将其转换为 Cartographer 需要的格式，以便进一步传递给轨迹构建器进行处理
void SensorBridge::HandleNavSatFixMessage(
    const std::string& sensor_id, const sensor_msgs::NavSatFix::ConstPtr& msg) {
  const carto::common::Time time = FromRos(msg->header.stamp);
  // 如果不是固定解,就加入一个固定的空位姿
  // 如果 GPS 解不可用（即没有固定解，STATUS_NO_FIX），则表示无法获得有效的位置信息。此时，代码向轨迹构建器添加一个空的位姿（absl::optional<Rigid3d>()）。Rigid3d() 表示没有有效的位姿数据。
  // 然后，函数提前返回，不再执行后续处理
  ...
  // 确定ecef原点到局部坐标系的坐标变换
  // ecef_to_local_frame_ 是一个存储坐标变换的可选值（absl::optional）。它用于存储从 ECEF（Earth-Centered, 
  // Earth-Fixed）坐标系到局部坐标系（通常是相对坐标系）的转换矩阵。
  // ComputeLocalFrameFromLatLong 是一个计算函数，它根据 GPS 数据中的经纬度（msg->latitude 和 msg->longitude）
  // 计算出一个局部坐标系的变换。这个局部坐标系一般是将地球表面的某个点作为原点，方便进行相对定位和导航。
  // 如果之前没有计算过该局部坐标系的转换（即 ecef_to_local_frame_ 不存在），就进行计算，并将结果存储在 ecef_to_local_frame_ 中。
...
// sensor_id：传感器标识符，表示当前处理的激光雷达数据来自哪个传感器。
// time：当前激光雷达扫描帧的时间戳。
// frame_id：当前激光雷达数据的坐标系标识。
// points：当前处理的激光雷达数据，包含了点云数据和每个点的强度。
// 将激光雷达数据分割成多个时间片段，每个片段单独处理，以便进行更精细的处理或合并
void SensorBridge::HandleLaserScan(
  ...
  // CHECK_LE: 小于等于
  // 数据校验：使用CHECK_LE宏来检查最后一个点的时间偏移是否不大于0。这是一种健壮性检查，确保数据是期望的格式
...
void SensorBridge::HandleRangefinder(
  ...
  // std::cout<<"sensor_to_tracking->translation():"<<sensor_to_tracking->translation()<<std::endl;
}
