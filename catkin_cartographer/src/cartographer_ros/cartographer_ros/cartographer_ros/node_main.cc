...
void Run() {
  // 初始化 tf2 变换缓冲区和用于监听和查询 tf （坐标变换） 的线程。
  // tf2_ros::TransformListener对象自动订阅/tf和/tf_static两个话题
  // /tf话题：这个话题用于发布坐标系间的变换信息，这些变换可能随着时间而变化。例如，一个移动机器人的位置相对于地图坐标系会不断更新。
  // /tf_static话题：这个话题用于发布那些不会随时间变化的坐标系变换信息，即所谓的静态变换。静态变换一旦定义，就假定它们在系统运行期间保持不变。例如，摄像头相对于机器人底盘的位置
  constexpr double kTfBufferCacheTimeInSeconds = 10.;
  tf2_ros::Buffer tf_buffer{::ros::Duration(kTfBufferCacheTimeInSeconds)};
  // 开启监听tf的独立线程
  tf2_ros::TransformListener tf(tf_buffer);
  // 定义 NodeOptions 和 TrajectoryOptions 对象，用于配置 SLAM 系统
  ...
  // 使用 std::tie 从配置文件加载配置到 node_options 和 trajectory_options。
  std::tie(node_options, trajectory_options) =
      LoadOptions(FLAGS_configuration_directory, FLAGS_configuration_basename);

  // MapBuilder类是完整的SLAM算法类
  // 包含前端(TrajectoryBuilders,scan to submap) 与 后端(用于查找回环的PoseGraph) 
  // 创建 MapBuilder 对象，它是 SLAM 算法的核心。包括前端（处理扫描到的子地图的匹配）和后端（用于闭环检测的位姿图）。
  ...
  // 初始化 Node 类的实例，由此 SLAM 系统开始处理传入的数据
  Node node(node_options, std::move(map_builder), &tf_buffer,
            FLAGS_collect_metrics);

  // 如果加载了pbstream文件, 就执行这个函数
  // 如果指定了加载 SLAM 状态的文件，则调用 LoadState 方法来加载已保存的状态。
