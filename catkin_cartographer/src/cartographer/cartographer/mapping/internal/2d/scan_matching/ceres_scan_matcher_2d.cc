...
// 实现了基于 Ceres 优化框架 的 2D 激光雷达扫描匹配算法，使用了多种残差项对点云与栅格地图进行优化，以求得较精确的机器人位姿。主要流程包括：
// 根据预测的先验位姿与地图数据定义优化问题。
// 添加多种残差项：
// 点云与地图的重叠程度（Occupied Space）。
// 位姿与目标位移的偏差（Translation Delta）。
// 位姿与目标旋转的偏差（Rotation Delta）。
// 使用 Ceres 进行非线性优化，得到校正后的机器人位姿
