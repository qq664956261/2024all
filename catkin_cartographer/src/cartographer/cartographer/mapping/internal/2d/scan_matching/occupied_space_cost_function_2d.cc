...
class OccupiedSpaceCostFunction2D {
 public:
  ...
  bool operator()(const T* const pose, T* residual) const {
    ...
    //使用了 Ceres Solver 中的 BiCubicInterpolator 类，创建了一个三次插值器对象 interpolator。
    // 插值器的作用是对二维栅格地图进行插值，计算给定坐标处的值（代价、概率等），即使坐标不正好位于栅格的中心点上。
    // 在扫描匹配中，点云的点经过坐标变换后，可能不完全落在栅格中心，而是位于栅格之间的位置。为了更精确地计算代价值，BiCubicInterpolator 对这些非整数坐标使用三次插值，获取更精确的栅格值。
  ...
  class GridArrayAdapter {
   public:
    ...
    void GetValue(const int row, const int column, double* const value) const {
      // 处于地图外部时, 赋予最大free值
      // 边界检查
      //即 row 或 column 的值超出地图范围（包括 kPadding）。
      //将 value 设置为 kMaxCorrespondenceCost（表示最大自由值，对应于最低占用概率）。
