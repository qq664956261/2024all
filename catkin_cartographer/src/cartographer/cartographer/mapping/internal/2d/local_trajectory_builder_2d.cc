 ...
 */
// 通过扫描匹配（scan matching）优化激光雷达数据的位姿估计。该函数采用两个主要的匹配算法：在线相关扫描匹配（correlative scan matching）和 Ceres 优化器进行精细调整。
std::unique_ptr<transform::Rigid2d> LocalTrajectoryBuilder2D::ScanMatch(
    const common::Time time, const transform::Rigid2d& pose_prediction,
    const sensor::PointCloud& filtered_gravity_aligned_point_cloud) {
  // 如果当前没有有效的子图，直接返回预测的位姿 pose_prediction，并且不进行匹配。
  if (active_submaps_.submaps().empty()) {
    return absl::make_unique<transform::Rigid2d>(pose_prediction);
  }
  // 使用active_submaps_的第一个子图进行匹配
  // 如果有子图存在，选择第一个子图 matching_submap 进行匹配。这里假设 active_submaps_ 是一个存储多个子图的容器，submaps().front() 返回第一个子图。
...
LocalTrajectoryBuilder2D::AddAccumulatedRangeData(
  ...
  // std::cout<<"non_gravity_aligned_pose_prediction.rotation():"<<non_gravity_aligned_pose_prediction.rotation().toRotationMatrix()<<std::endl;
  // std::cout<<"gravity_alignment.rotation():"<<(non_gravity_aligned_pose_prediction
  // * gravity_alignment.inverse()).rotation().toRotationMatrix()<<std::endl;
  ...
  T.block<3, 3>(0, 0) = pose_estimate.rotation().toRotationMatrix();
  T.block<3, 1>(0, 3) = pose_estimate.translation();
  std::string kittipath = "/home/zc/kitti_carto.txt";
  std::ofstream out_kitti(kittipath, std::ios::app);

  out_kitti << std::fixed << std::setprecision(9) << T(0, 0) << " " << T(0, 1)
            << " " << T(0, 2) << " " << T(0, 3) << " " << T(1, 0) << " "
