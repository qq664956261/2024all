...
namespace mapping {
// ProbabilityGrid 用来表示一个 2D 占用栅格地图，每个栅格的值存储了该位置被占用的概率
/**
 ...
 */
// ProbabilityGrid 的构造函数接受地图的坐标限制和一个转换表。conversion_tables 用来在不同的表示方式之间转换占用栅格值（例如概率值和对应的代价值）。
...
// 用于设置某个栅格的占用概率值。它的主要目的是修改栅格中某个特定位置（通过 cell_index 确定）对应的概率值
void ProbabilityGrid::SetProbability(const Eigen::Array2i& cell_index,
                                     const float probability) {
  // 获取对应栅格的引用
//   mutable_correspondence_cost_cells() 返回一个指向栅格数据的可修改指针，ToFlatIndex(cell_index) 将二维索引 cell_index 转换为一维的索引位置，指向栅格数据中的某个具体位置。
// uint16& cell 是该栅格单元的引用，通过它可以直接修改该栅格的值。
  ...
  // 为栅格赋值 value
  // 将输入的概率值转换为栅格的代价值并赋值给该栅格
  cell =
      CorrespondenceCostToValue(ProbabilityToCorrespondenceCost(probability));
  // 更新bounding_box
  //更新地图的 known_cells_box。
  //mutable_known_cells_box() 返回一个可修改的边界框，表示当前已知的栅格区域。extend 方法会更新该边界框，确保新设置的栅格索引被包含在已知区域内。
...
// 使用查找表对指定栅格进行栅格值的更新
// 用于根据查找表更新栅格的值。它的主要目的是根据传入的查找表对指定位置的栅格进行更新，并确保每个栅格不会被重复更新
bool ProbabilityGrid::ApplyLookupTable(const Eigen::Array2i& cell_index,
                                       const std::vector<uint16>& table) {
  //检查传入的 table 的大小是否为 kUpdateMarker，这是一个用于标记表格大小是否符合要求的检查。
  ...
  // 对处于更新状态的栅格, 不再进行更新了
  // 检查该栅格单元是否已经被更新过。
  // 如果栅格的当前值大于等于 kUpdateMarker，则表示该栅格已经被更新过
