...
void Node::Init()
{
  ...
  path_pub_ = node_handle_.advertise<nav_msgs::Path>(name_of_node_ + "/trajectory", 10);
...
void Node::LoadOrbParameters(ORB_SLAM2::ORBParameters &parameters)
{
  ...
  std::cout << "parameters.minThFAST:" << parameters.minThFAST << std::endl;
  ...
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_fx", parameters.fx);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_fy", parameters.fy);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_cx", parameters.cx);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_cy", parameters.cy);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_k1", parameters.k1);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_k2", parameters.k2);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_p1", parameters.p1);
  got_cam_calibration &= node_handle_.getParam("/orb_slam2_mono/camera_p2", parameters.p2);

  std::cout << "parameters.fx:" << parameters.fx << std::endl;

  if (!got_cam_calibration)
  {
    std::cout << "sensor_:" << sensor_ << std::endl;
    std::cout << "name_of_node_:" << name_of_node_ << std::endl;
