...
int LabelClustering(const cv::Mat &ipm_semantic, cv::Mat &ipm_label, cv::Mat &stats, cv::Mat &centroids)
{
	...
	// 	ipm_semantic_stop.setTo(255, ipm_semantic == 5)：将 ipm_semantic 中值为5（停止线标记）的像素位置置为255，即白色。
	// ipm_semantic_temp.setTo(0, ipm_semantic == 5)：将 ipm_semantic_temp 中值为5的像素位置置为0，即黑色，以便后续连通区域分析。
	ipm_semantic_stop.setTo(255, ipm_semantic == 5);
	ipm_semantic_temp.setTo(0, ipm_semantic == 5);
	// 进行连通区域分析，得到所有区域的标签图、统计信息和质心。
	...
	// 将停止线区域的标签图像素更新为整体标签图像的行数减去停止线区域的行数加上当前停止线区域的索引 i
...
// 计算了在逆透视映射（IPM）处理过程中，对于一个点在空间中位置和方向的估计不确定性。
...
{
	// 使用 LabelClustering 函数将 ipm_class 分类图像聚类成不同的实例区域，并获得每个区域的统计信息和质心。
	...
	while (it != patches_temp.end())
	{
		...
		if (it->points.size() < config.patch_min_size || (it->road_class == PatchType::DASHED && out_range) || (it->road_class == PatchType::GUIDE && out_range))
		{
			it = patches_temp.erase(it);
			continue;
		}
	...
	for (int i = 0; i < patches_temp.size(); i++)
	{
		...
		if (patch.eigen_value[0] / patch.eigen_value[1] > 2 && (patches_temp[i].road_class == PatchType::SOLID || patches_temp[i].road_class == PatchType::STOP))
		{
			...
			else
			{
					 ...
					 ll < patch.eigen_value[0] * 2 / (0.5 / reso_temp); ll++)
				{
					...
					if (pt_img(1) > (patch.top + patch.height))
						continue;

					double valid_distance = 20;
					if (patches_temp[i].road_class == PatchType::SOLID)
						valid_distance = config.patch_solid_max_dist;
					else if (patches_temp[i].road_class == PatchType::STOP)
						valid_distance = config.patch_stop_max_dist;
		...
		if (patch->road_class == PatchType::GUIDE || patch->road_class == PatchType::DASHED)
		{
			...
			if (min_dist < 10000)
			{
				...
				for (int jj = 0; jj < patch->points.size(); jj++)
				{
					...
					{
						ll = pt.x();
						ll_support = patch->points[jj];
					}
					if (pt.x() > rr)
					{
						rr = pt.x();
						rr_support = patch->points[jj];
					}
					if (pt.y() < tt)
					{
						tt = pt.y();
						tt_support = patch->points[jj];
					}
					if (pt.y() > bb)
					{
						bb = pt.y();
						bb_support = patch->points[jj];
					}
...
//将每个补丁的像素点从逆透视映射（IPM）空间转换到度量空间，并计算补丁的一些度量特征。
int RoadInstancePatchFrame::generateMetricPatches(const SensorConfig &config, const gv::IPMProcesser &ipm)
{
	...
	for (auto iter_class : patches)
	{
		for (int i = 0; i < iter_class.second.size(); i++)
		{
			...
			for (int j = 0; j < patch->points.size(); j++)
			{
				patch->points_metric[j] = cam.tic + cam.Ric * ipm.IPM2Metric(cv::Point2f(patch->points[j].x(), patch->points[j].y()));
			}
			...
			if (patch->line_valid)
			{
				...
				{
					patch->line_points_metric[j] = cam.tic + cam.Ric * ipm.IPM2Metric(cv::Point2f(patch->line_points[j].x(), patch->line_points[j].y()));
				}
			}
			...
			if (patch->road_class == PatchType::DASHED || patch->road_class == PatchType::GUIDE)
			{
				...
				{
					patch->b_point_metric[ii] = cam.tic + cam.Ric * ipm.IPM2Metric(cv::Point2f(patch->b_point[ii].x(), patch->b_point[ii].y()));
				}
			}
			...
			if (iter_class.first == PatchType::DASHED && patch->h() > config.patch_dashed_min_h && patch->h() < config.patch_dashed_max_h && patch->mean_metric(1) < config.patch_dashed_max_dist)
				patch->valid_add_to_map = true;
			else if (iter_class.first == PatchType::GUIDE && patch->h() > config.patch_guide_min_h && patch->h() < config.patch_guide_max_h && patch->mean_metric(1) < config.patch_guide_max_dist)
				patch->valid_add_to_map = true;
