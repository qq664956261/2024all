...
import os
import sys
sys.path.append("/data/03_code/00_slam/scancontext_tro")
...
def distance_sc(sc1, sc2):
    ...
    for i in range(num_sectors):
        ...
        if(sum_of_cos_sim == 0 or num_col_engaged == 0):
            sim_for_each_cols[i] = 0
        else:
            sim_for_each_cols[i] = sum_of_cos_sim / num_col_engaged
  
        if (sim_for_each_cols[i] == 1.0):
            print("sum_of_cos_sim: ", sum_of_cos_sim, " num_col_engaged: ", num_col_engaged)

    sim = max(sim_for_each_cols)
    sim_index = np.argmax(sim_for_each_cols)

    dist = 1 - sim

    return dist, sim_index



if __name__ == "__main__":
    from python_test.make_sc_example import *
