...
#include "relocalization/kdtree.h"
...
namespace aiper_relocalization_ns
{
    ...
    bool KdTree::GetClosestPoint(const PointType &pt, std::vector<int> &closest_idx, std::size_t k)
    {
        if (k > size_)
        {
    ...
    bool KdTree::GetClosestPointMT(const CloudPtr &cloud, std::vector<std::pair<size_t, size_t>> &matches, std::size_t k)
    {
