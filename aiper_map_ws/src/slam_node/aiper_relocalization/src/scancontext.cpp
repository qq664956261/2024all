...
MatrixXd SCManager::makeScancontext(mypcl::PointCloud<SCPointType> &_scan_down)
{
...
void SCManager::makeAndSaveScancontextAndKeys(mypcl::PointCloud<SCPointType> &_scan_down)
{
...
void SCManager::makeAndSaveKeys(Eigen::MatrixXd _sc_mat)
{
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(_sc_mat);
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(_sc_mat);
    std::vector<float> polarcontext_invkey_vec = eigen2stdvec(ringkey);

    polarcontexts_.push_back(_sc_mat);
    polarcontext_invkeys_.push_back(ringkey);
    polarcontext_vkeys_.push_back(sectorkey);
    polarcontext_invkeys_mat_.push_back(polarcontext_invkey_vec);
}

void SCManager::makeAndSaveScancontextAndKeysSource(mypcl::PointCloud<SCPointType> &_scan_down)
{
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1
    Eigen::MatrixXd mat(30, 180);
    mat = sc.block<30, 180>(0, 0);
    sc = mat;
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(sc);
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(sc);
    std::vector<float> polarcontext_invkey_vec = eigen2stdvec(ringkey);

    polarcontexts_source_ = sc;
    polarcontext_invkeys_source_ = ringkey;
    polarcontext_vkeys_source_ = sectorkey;
    polarcontext_invkeys_mat_source_ = polarcontext_invkey_vec;
}
...
std::pair<float, float> SCManager::detectLoopClosureIDOneFrame(Eigen::MatrixXd _scd)
{
    int loop_id{-1}; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")

    auto curr_key = polarcontext_invkeys_mat_source_; // current observation (query)
    auto curr_desc = polarcontexts_source_;           // current observation (query)
    ...
    MatrixXd polarcontext_candidate = _scd;
    ...
    // std::cout << "yaw_diff_rad:" << yaw_diff_rad << std::endl;
    std::pair<float, float> result{candidate_dist, yaw_diff_rad};

    return result;

} // SCManager::detectLoopClosureIDOneFrame
