...
/// 常用的数学函数
namespace aiper_relocalization_ns::math
{
     ...
     *
     * std::accumulate，这个算法接收一个范围内的元素（通常由迭代器指定的序列），
     * 一个初始值，以及一个二元操作函数（可以是函数指针、函数对象或lambda表达式）
     * // Getter被用在了std::accumulate的lambda表达式里，
    // 作为获取data中元素的函数，这是对Getter类型的一个重要提示。
    // 因为是模板函数，Getter的具体类型在编译时期决定，
    // 这意味着Getter可以是任何可以调用的类型，包括函数指针、函数对象或者lambda表达式，
    // 其主要条件是，它必须能接受data的元素类型作为参数，并返回可以用来进行加法和减法操作的类型。
    // 换句话说，Getter必须是一个Unary Function，其输入参数类型与data元素类型一致，返回值类型与D兼容
    // D::Zero().eval()作为std::accumulate函数的一个参数，其作用是提供一个累加器的初始值
    const D& sum是lambda的第一个参数，按照std::accumulate算法的定义，这是累加值（accumulator），它是前一次操作的输出，或者在第一次迭代时是初始值D::Zero().eval()。
    const auto& data是lambda的第二个参数，它对应于当前正在处理的集合中的元素。
     */
    ...
    cov_diag = std::accumulate(data.begin(), data.end(), D::Zero().eval(),
                               [&mean, &getter](const D& sum, const auto& data) -> D {
                                   return sum + (getter(data) - mean).cwiseAbs2().eval();//[-1, 2, -3],v.cwiseAbs2()计算数组v中每个元素的平方绝对值。输出结果v_abs2将是[1, 4, 9]
                               }) / (len - 1);
        // clang-format on
    }
                 ...
                 double eps = 0.2)
    {
        ...
        // check eps
        // 遍历每个点并计算每个点到拟合直线的距离，这个距离通过计算点与原点的差向量与拟合直线方向向量的叉积的模长得到。
        // 如果任何一个点到直线的距离的平方超过给定的误差容忍度eps，函数返回false，表示拟合不满意
        // dir为单位向量，叉积为面积，叉积的模长为面积，面积除以底边长为高，即点到直线的距离，底边长为1，所以直接取模长即可
