...
#include "relocalization/common/eigen_types.h"
#include "relocalization/common/point_types.h"
...
class KdTree {
   public:
    ...
    /// 获取k最近邻
    bool GetClosestPoint(const PointType& pt, std::vector<int>& closest_idx, std::size_t k = 5);

    /// 并行为点云寻找最近邻
    bool GetClosestPointMT(const CloudPtr& cloud, std::vector<std::pair<size_t, size_t>>& matches, std::size_t k = 5);
