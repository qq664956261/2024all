...
#include "relocalization/nanoflann.hpp"
#include "relocalization/KDTreeVectorOfVectorsAdaptor.h"
...
using SCPointType = mypcl::PointXYZI; // using xyz only. but a user can exchange the original bin encoding function (i.e., max hegiht) to max intensity (for detail, refer 20 ICRA Intensity Scan Context)
...
public: 
    ...
    Eigen::MatrixXd makeScancontext( mypcl::PointCloud<SCPointType> & _scan_down );
    ...
    // User-side API
    void makeAndSaveScancontextAndKeys( mypcl::PointCloud<SCPointType> & _scan_down );
    void makeAndSaveKeys(Eigen::MatrixXd _sc_mat);
    void makeAndSaveScancontextAndKeysSource(mypcl::PointCloud<SCPointType> &_scan_down);
    ...
    std::pair<float, float> detectLoopClosureIDOneFrame(Eigen::MatrixXd _scd );
...
public:
    ...
    // SC_DIST_THRES = 0.2; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold / if not, recommend 0.1-0.15
     SC_DIST_THRES = 0.3; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold / if not, recommend 0.1-0.15
    ...
    Eigen::MatrixXd polarcontexts_source_;
    Eigen::MatrixXd polarcontext_invkeys_source_;
    Eigen::MatrixXd polarcontext_vkeys_source_;
    std::vector<float> polarcontext_invkeys_mat_source_;
