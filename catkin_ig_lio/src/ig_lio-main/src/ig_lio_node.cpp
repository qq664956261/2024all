...
void ImuCallBack(const sensor_msgs::Imu::ConstPtr& msg_ptr) {
...

  {
    ...
    if (enable_acc_correct) {
      ...
      imu_msg.linear_acceleration.x = imu_msg.linear_acceleration.x * 1;
      imu_msg.linear_acceleration.y = imu_msg.linear_acceleration.y * 1;
      imu_msg.linear_acceleration.z = imu_msg.linear_acceleration.z * 1;
    }
  ...
  if (!measurement_pushed) {
    if (!process_lidar) {
      ...
      *cloud_sort = *cloud_buff.front().second;
      ...
      // {
      //   std::cout<<"cloud_sort[i].curvature:"<<std::to_string(cloud_sort->points[i].curvature)<<std::endl;
      // }
      // std::cout<<"*********************************************"<<std::endl;
      ...
      if (!local_sensor_measurement.cloud_ptr_->points.empty()) {
        //除以 1000 的原因是因为激光点云的时间信息通常是以“毫秒”为单位存储的，而 lidar_start_time_ 的时间戳需要用“秒”作为单位
      ...
      // 这一部分判断激光点云中点的数量是否小于等于 1 个（即点云太少）。如果是的话，无法通过点云中的点来推测扫描的结束时间。
      // 由于点云数量不足，代码将扫描的结束时间 (lidar_end_time) 设为激光雷达开始时间 (lidar_start_time_) 加上平均扫描时间 (lidar_mean_scantime)。
      ...
      //如果点云中的最后一个点的时间戳（通过 curvature 字段获取的时间信息，单位是毫秒）小于平均扫描时间的一半，
      //那么认为这个点的时间戳有问题或不合理，因此使用 lidar_start_time_ 加上平均扫描时间来设置结束时间。
