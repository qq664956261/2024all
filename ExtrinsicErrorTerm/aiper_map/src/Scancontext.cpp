...
std::pair<int, float> SCManager::detectLoopClosureID(void)
{
    ...
    {
        e_curr_key(i, 0) = curr_key[i];
        std::cout << "e_curr_key(i, 50):" << e_curr_key(i, 50) << std::endl;
    }
    Eigen::Matrix<float, 200, 1> e_polarcontext_invkeys_mat;
    for (int i = 0; i < PC_NUM_RING; i++)
    {
        e_polarcontext_invkeys_mat(i, 0) = polarcontext_invkeys_mat_[nn_idx][i];
        std::cout << "e_polarcontext_invkeys_mat(i, 50):" << e_polarcontext_invkeys_mat(i, 50) << std::endl;
    }
    auto vkey_diff = e_curr_key - e_polarcontext_invkeys_mat;
    double cur_diff_norm = vkey_diff.norm();
    std::cout << "cur_diff_norm:" << cur_diff_norm << std::endl;
    cv::Mat img1;
    cv::eigen2cv(e_curr_key, img1);
    img1.convertTo(img1, CV_8UC1, 255.0);
    cv::imwrite("./img1.png", img1);
    cv::Mat img2;
    cv::eigen2cv(e_polarcontext_invkeys_mat, img2);
