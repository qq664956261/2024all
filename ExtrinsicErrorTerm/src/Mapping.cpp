...
void Mapping::map()
{
    ...
    for (int i = 0; i < _p_cloud_pose.size(); i++)
    {
        ...
        if (loop_index > -1)
        {
            ...
            options.min_effective_pts_ = 20;
            options.eps_ = 1e-8;
            options.max_line_distance_ = 0.25;
            options.use_initial_translation_ = true;
            // sad::Icp3d icp;
            sad::Icp3d icp(options);
            mypcl::voxelGridFilter(*cloud,*cloud, 0.1);
            mypcl::voxelGridFilter(*target,*target, 0.1);
            icp.SetSource(cloud);
            ...
            std::cout << "result_loop:" << result_loop << std::endl;
            std::cout << "T_target:" << T_target << std::endl;
            std::cout << "T_source:" << T_source << std::endl;
            // result_loop(0, 1) = 0.04;
            // result_loop(1, 0) = -0.04;
            *out_loop += *target;
...
void Mapping::LoopClosure(const int index1, const int index2, const Eigen::Matrix4d &pose1, const Eigen::Matrix4d &pose2)
{
    ...
    else
    {
        ...
        e->setVertex(0, vp0); // first viewpoint
        e->setVertex(1, vp1); // second viewpoint
        e->information() = 10000 * Eigen::Matrix<double, 6, 6>::Identity();
        optimizer.addEdge(e);
        optimizer.initializeOptimization();
        optimizer.computeActiveErrors();
...
int Mapping::DetectLoopClosure(const Eigen::Matrix4d &pose, double &time_stamp)
{
    ...
    for (int i = 0; i < _keyframes.size(); i++)
    {
        ...
        if (distance < _distance_threshold * 0.6 && std::abs(time_stamp * 1e-6 - _keyframes[i].second.first * 1e-6) > 40)
        {
            std::cout << "pose" << pose << std::endl;
