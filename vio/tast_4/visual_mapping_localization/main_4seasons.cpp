...
std::vector<std::pair<std::string, std::string>> read_img_path(const std::string &path1, const std::string &path2, const std::string &list, std::vector<Eigen::Matrix4d> &T)
{
    ...
    {
        std::string img1 = path1 + line;
        img1.erase(147);
        std::string img2 = path2 + line;
        img2.erase(147);
        img_list.emplace_back(img1 + ".png", img2 + ".png");
        Eigen::Matrix4d T_;
        Eigen::Vector3d t;
        Eigen::Quaterniond q;
        for (int i = 0; i < 6; i++)
...
int main()
{
    ...
    std::string img_path = "/home/zc/data/vio/recording_2020-03-24_17-36-22_stereo_images_undistorted/recording_2020-03-24_17-36-22/undistorted_images/cam0/";
    std::string img_path2 = "/home/zc/data/vio/recording_2020-03-24_17-36-22_stereo_images_undistorted/recording_2020-03-24_17-36-22/undistorted_images/cam1/";
    std::string img_list_path = "/home/zc/code/vio/orbslam+superpoint/ORB_SLAM3-master/kf_4y.txt";
    std::vector<Eigen::Matrix4d> T;
    auto img_list = read_img_path(img_path, img_path2, img_list_path, T);

    std::string cam_param_path = "/home/zc/code/vio/orbslam+superpoint/ORB_SLAM3-master/Examples/Stereo/4seasons.yaml";
    Camera cam1, cam2;
    Eigen::Matrix4d T12;
    ...
    std::shared_ptr<FeatureDetection> detection =
        std::make_shared<FeatureDetection>(SuperPoint, "/home/zc/code/vio/tast_3/OpenVINO/learned_features_inference-openvino/weight/sp",
                                           8, 500, 512, 512);
