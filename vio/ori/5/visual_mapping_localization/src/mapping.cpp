...
//                        }
                        ...
                        // TODO 1 fuse the map points
                        // 1. 遍历所有连接帧
                        // 2. 判断连接帧对应的特征状态 status, 判断当前地图点和连接帧对应的地图点是否一致
                        // 3. 如果不一致, 开始融合
                        //   4.1 从map中删除连接帧对应的地图点
                        //   4.2 将连接帧对应的地图点融合到当前地图点，分为两种情况
                        //     4.2.1 当前地图点为空, 直接融合
                        //     4.2.2 当前地图点不为空, 将连接帧对应的地图点融合到当前地图点
