...
//        std::cout<<"edges size: "<<edges.size()<<std::endl;
        // 开始优化
        // TODO 修改为多次迭代优化，将误差大的边设为outlier
        int num = 2;// 迭代次数
        status.resize(frames.size(), 0);// 0表示outlier
        ...
        for (size_t it= 0;it < num; it ++)
        {
            ...
            status[0] = 1;// 确保第一个地图点不是outlier
            // 将误差大于阈值的边设为outlier，阈值为5.991
            // 95%的误差落在2倍标准差内，2倍标准差对应的卡方分布值为5.991
            // 只需要修改这里
