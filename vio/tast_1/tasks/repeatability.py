...
def val_key_points(kps0, kps1, warp01, warp10, th: int = 3):
    ...
    print("kps0:",kps0)
    print("kps0:",kps0.size())
    print("kps1:",kps1.size())
    # ==================================== covisible keypoints
    # 调用warp函数，将图像0中的特征点投影到图像1中
    kps0_cov, kps01_cov, _, _ = warp(kps0, warp01)
    #print("kps0_cov:",kps0_cov)
    print("kps0_cov:",kps0_cov.size())
    ...
    min_distances, _ = dist.min(dim=1)

    print("dist:",dist.size())
    print("th",th)
    print("warp01['resize']:",warp01['resize'])

    # 4. 正确调用 mutual_argmin 函数，得到最小距离的索引
    index = mutual_argmin(dist)
    print("index:",index)
    
    # 5. 得到最小距离
    shape = (len(index[0]))  
    dis = torch.zeros(shape)
    print("len(index[0]):",len(index[0]))
    print("len(index[1]):",len(index[1]))
    for i in range(len(index[0])):
        dis[i] = dist[index[0][i].item()][index[1][i].item()]
        #print("index[i][i]:",dist[index[0][i].item()][index[1][i].item()])
    ...
    print("dist_mask:",dist_mask.size())
    gt_num = dist_mask.sum().cpu()
    mean_error = dist[dist_mask].cpu().numpy().mean()
    errors = dist
    # 返回的一些关联变量
    # gt_num 正确关联点的数量
    ...
    print("gt_num:",gt_num)
    print("gt_num / num_feat:",gt_num / num_feat)
