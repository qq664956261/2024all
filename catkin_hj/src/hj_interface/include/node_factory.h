...
class ManagerNode {
 ...
 private:
  ...
  std::unordered_map<std::string, std::pair<std::string, std::map<uint32_t, std::shared_ptr<ros::Publisher>>>> publishers_;
...
HJPublisher HJAdvertise(const std::string& topic, uint32_t queue_size, bool latch) {
  ...
  if (it == g_manager_node_ptr->publishers_.end()) {
    ...
    temp[g_manager_node_ptr->publishers_index_] =
        std::make_shared<ros::Publisher>(g_manager_node_ptr->node_handle_ptr_->advertise<T>(topic, queue_size, latch));
    g_manager_node_ptr->publishers_[topic] = std::make_pair(typeid(T).name(), temp);
  } else {
    if (it->second.first != (typeid(T).name())) {
      ...
      it->second.second[g_manager_node_ptr->publishers_index_] =
          std::make_shared<ros::Publisher>(g_manager_node_ptr->node_handle_ptr_->advertise<T>(topic, queue_size, latch));
    }
  }
  g_manager_node_ptr->publishers_index_++;
  return HJPublisher(topic, g_manager_node_ptr->publishers_[topic].second[g_manager_node_ptr->publishers_index_ - 1],
                     g_manager_node_ptr->publishers_index_ - 1);
  //  return
}
