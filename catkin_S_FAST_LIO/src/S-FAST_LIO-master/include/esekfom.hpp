...
#pragma omp parallel for
			...
			for (int i = 0; i < feats_down_size; i++) //遍历所有的特征点
			{
				...
				{
					// d = (Ax + By + Cz + D)/sqrt(A^2 + B^2 + C^2)其中A^2 + B^2 + C^2 = 1
			...
			// 雅可比矩阵H和残差向量的计算
			// 十二维为p,R,外参
			// 其余的状态变量（如速度、偏置、重力等）在当前的测量模型中并未影响点到平面的距离，因此：
			// 残差对这些变量的导数为 0。
			// 这些变量的优化和调整可能会在其他观测模型中进行，例如 IMU 观测模型或动态约束。
			// 因此，在当前的测量模型中，只需要优化与点到平面距离相关的 12 维状态
			ekfom_data.h_x = MatrixXd::Zero(effct_feat_num, 12);
			ekfom_data.h.resize(effct_feat_num);
			// 残差计算：点到平面的距离 d 作为观测残差。
			// 雅可比矩阵 H：残差对状态变量的偏导数，描述残差随状态变化的敏感度。
			for (int i = 0; i < effct_feat_num; i++)
			{
				...
				//H = de/dp, de/dR, de/dR_L_I, de/dt_L_I
				if (extrinsic_est)
				{
