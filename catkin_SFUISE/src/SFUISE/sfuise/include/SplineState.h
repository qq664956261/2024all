...
// itpQuaternion的函数能够进行四元数的插值计算，并且同时还能够给出一个矩阵Jacobian的导数值，来完成对运动的模拟和模拟结果的微调
...
// prepareInterpolation函数的主要目的是计算并准备好所有用于插值计算的参数。它接收时间t_ns，
// 空闲节点集合knots_idle，节点集合knots作为输入，然后更新如下参数的值：插值的起始索引idx0，插值的时间段u，
// 以及用于插值计算的节点集合cps。
    ...
    {
        int64_t t_ns_rel = t_ns - start_t_ns;//首先计算相对起始时间start_t_ns的相对时间t_ns_rel
        int idx_l = floor(double(t_ns_rel) / double(dt_ns));//然后找到时间窗口idx_l和idx_r，其中idx_l是相对起始时间左边的节点的索引，而idx_r是右边的节点的索引。
        idx_r = idx_l + 1;
        idx0 = std::max(idx_l - 2, 0);//确定插值起始的索引idx0。这个值为时间窗口idx_l减去2和0之间的最大值
        //idx_l在0和1之间的情况
        ...
        u = (t_ns - start_t_ns - idx_l * dt_ns) / double(dt_ns);//最后，计算插值的时间段u，即目标时间到时间窗口左边节点的时间长度与节点间隔的占比。
    }
// baseCoeffsWithTime函数主要用于根据给定时间来计算Hermite插值基函数的系数。函数的输入包括一个固定大小为4的向量res_const和一个时间参数t
    template <int Derivative, class Derived>
    static void baseCoeffsWithTime(const Eigen::MatrixBase<Derived>& res_const, double t)
    {
        //函数首先进行一次静态断言，用于确保输入向量res_const的大小为4
        EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(Derived, 4);
        //由于Eigen库中的常量矩阵不允许更改值，所以需将res_const向量转化为非常量res，以便之后进行赋值操作
        ...
        // 函数使用循环对向量res的其余元素进行赋值，其值以时间t的递增幂为系数。
        // 这里的base_coefficients(Derivative, j)方法应该是用来得到在Derivative点附近，
        // j阶Hermite插值的基函数的系数，然后乘以时间ti累积得到全局基函数的系数
        double ti = t;
...
// computeBaseCoefficients函数用于计算基本系数。该方法返回一个4x4的矩阵.
// 这个函数的主要作用就是计算三次Hermite插值的基函数的系数。通过这样的计算，
// 我们可以很方便的得到不同阶乘的Hermite插值基函数的值。
// 注意，经过这个函数的计算之后，base_coeff矩阵中的每行表示的应该是同一个Hermite插值基函数的不同时间导数的系数。
// 如果我们想要用这个矩阵来计算Hermite插值，我们需要在后续的处理中再乘以一个向量，该向量的元素是不同时间变量的相应阶乘。
    static Eigen::Matrix4d computeBaseCoefficients()
    {
        Eigen::Matrix4d base_coeff;
        // 首先创建一个4x4的零矩阵base_coeff，然后将第一行赋值为1
        base_coeff.setZero();
