...
class SplineFusion
{
  ...
  private:
    ...
    void readParameters(ros::NodeHandle& nh)
    {
        ...
        //if_uwb_only = true;
        std::cout<<"if_uwb_only:"<<if_uwb_only<<std::endl;
        param.if_opt_g = true;
...
// 这个函数通过不断更新以保障data_window中的数据反映了定义时间窗口内的最新测量信息，并同时也会对data_buff进行清理，移除那些不再处于时间范围内的旧数据
    ...
    bool initialization()
    {
        ...
        if (initialize_control_point) {
            ...
            if (param_set && if_anchor_ini) {
                ...
                std::cout<<"calib_param.t_nav_uwb:"<<param.t_nav_uwb_init<<std::endl;
                std::cout<<"calib_param.q_nav_uwb:"<<param.q_nav_uwb_init.w()<<std::endl;
                std::cout<<"calib_param.q_nav_uwb:"<<param.q_nav_uwb_init.x()<<std::endl;
                std::cout<<"calib_param.q_nav_uwb:"<<param.q_nav_uwb_init.y()<<std::endl;
                std::cout<<"calib_param.q_nav_uwb:"<<param.q_nav_uwb_init.z()<<std::endl;
...
// 这个applyIncFull函数的作用是把一个增量向量inc_full应用到局部样条曲线上，并根据这个增量更新曲线的姿态和（如果有的话）偏置，重力和传感器与导航系统之间的变换。
