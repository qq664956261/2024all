...
if __name__ == "__main__":
    ...
    for k in range(1,K):               # k = 1 ~ K-1
        ...
        if uwb_check:            # if we have UWB measurement
            ...
            # 创建包含当前位置和误差信息的DataFrame
            df = pd.DataFrame({
                'pos_x': [pos_x],
                'pos_y': [pos_y],
                'pos_z': [pos_z]
            })
                        # 检查文件是否存在以决定是否写入headers
            if not os.path.isfile(csv_file_path):
                df.to_csv(csv_file_path, mode='w', index=False, header=True)  # 创建新文件，并写入表头
            else:
                df.to_csv(csv_file_path, mode='a', index=False, header=False)  # 以追加模式写入新数据
