...
VQFParams::VQFParams()
    ...
    , biasSigmaInit(0.5)//0.5
    , biasForgettingTime(5)//100
    , biasClip(5.0)//2
#ifndef VQF_NO_MOTION_BIAS_ESTIMATION
    , biasSigmaMotion(0.1)
    , biasVerticalForgettingFactor(0.0001)
#endif
    , biasSigmaRest(0.0003)//0.03
    , restMinT(1.5)
    , restFilterTau(0.5)//0.5
    , restThGyr(1)//2
    , restThAcc(0.5)
    , magCurrentTau(0.05)
    , magRefTau(20.0)
...
{
    // rest detection
    // 首先检测是否启用了 休息偏置估计（params.restBiasEstEnabled）或 磁场干扰排除（params.magDistRejectionEnabled）。
    if (params.restBiasEstEnabled || params.magDistRejectionEnabled) {
        ...
        // 设置一个阈值 biasClip，用于判断是否超过偏置限制。然后根据偏差的大小判断当前状态是否为休息状态：
    ...
    // remove estimated gyro bias从陀螺仪原始数据中去除先前估计的偏置
    vqf_real_t gyrNoBias[3] = {gyr[0]-state.bias[0], gyr[1]-state.bias[1], gyr[2]-state.bias[2]};

    // gyroscope prediction step
    // 计算去除偏置后的陀螺仪数据的范数（gyrNorm），用于表示旋转速度。
    vqf_real_t gyrNorm = norm(gyrNoBias, 3);
    // 计算旋转角度 angle，等于旋转速度乘以采样时间（coeffs.gyrTs）
    vqf_real_t angle = gyrNorm * coeffs.gyrTs;
    // 如果 gyrNorm 大于一个非常小的值（EPS，表示避免数值不稳定的误差），则进行四元数更新：
    if (gyrNorm > EPS) {
        //q = cos(θ/2) + sin(θ/2)(ux * i + uy * j + uz * k)  θ是旋转角度，u旋转轴的单位向量
...
void VQF::updateAcc(const vqf_real_t acc[3])
{
    ...
    // rest detection休息状态检测
    // 使用滤波器对加速度数据进行滤波，得到低频成分 state.restLastAccLp。
    // 计算原始加速度与低频加速度之间的偏差平方和，以检测是否处于静止状态。
    // 如果偏差大于阈值，则重置静止时间 state.restT，并设置 state.restDetected 为 false；
    // 否则，增加静止时间并判断是否满足最小静止时间 params.restMinT，若满足，则设置 state.restDetected 为 true。
    ...
    // 使用陀螺仪四元数 state.gyrQuat 对加速度矢量进行旋转，将其转换到惯性系中。
    // 对转换后的加速度矢量 accEarth 进行滤波，以去除高频噪声。
    ...
    //accQuat
    std::cout<<"lastAccLp:"<<state.lastAccLp[0]<<" "<<state.lastAccLp[1]<<" "<<state.lastAccLp[2]<<std::endl;
    std::cout<<"accQuat:"<<state.accQuat[0]<<" "<<state.accQuat[1]<<" "<<state.accQuat[2]<<std::endl;
    quatRotate(state.accQuat, state.lastAccLp, accEarth);
    normalize(accEarth, 3);
    std::cout<<"2accEarth:"<<accEarth[0]<<" "<<accEarth[1]<<" "<<accEarth[2]<<std::endl;
    // inclination correction
    //cos(θ) = (v1 * v2)/(|v1| * |v2|)
    //= (accEarth * (0,0,-1))/(|accEarth| * |（0，0，-1）|)
    //= -accEarth[2]
    // cos(θ/2) = √（(1 + cos(θ))/2） = √((1 -accEarth[2])/2)
    vqf_real_t accCorrQuat[4];
    //vqf_real_t q_w = sqrt((accEarth[2]+1)/2);//(0,0,1)
    vqf_real_t q_w = sqrt((-accEarth[2]+1)/2);//(0,0.-1)
    if (q_w > 1e-6) {
        accCorrQuat[0] = q_w;
    ...
    normalize(state.accQuat, 4);
           std::cout << "accQuat: "  << ", quat: [" << accCorrQuat[0] << ", " << accCorrQuat[1] << ", " << accCorrQuat[2] << ", " << accCorrQuat[3] << "]" << std::endl;
    ...
    if (params.restBiasEstEnabled) {
        ...
        if (state.restDetected) {
            ...
            std::cout<<"state.bias[2]:"<<state.bias[2]<<std::endl;
            // step 4: P = P - K R P
...
{
     std::cout<<"state.magRefDip:"<<state.magNormDip[1]<<std::endl;
    // ignore [0 0 0] samples
    if (mag[0] == vqf_real_t(0.0) && mag[1] == vqf_real_t(0.0) && mag[2] == vqf_real_t(0.0)) {
        std::cout<<"ignore [0 0 0] samples"<<std::endl;
        return;
    }
    ...
    std::cout<<"state.delta:"<<state.delta<<std::endl;
...
// 这个函数的实现基于 二阶 IIR 滤波器（Infinite Impulse Response） 的差分方程
...
// 函数的作用是对输入向量 x 进行滤波操作，支持多通道输入。滤波操作基于一维差分方程实现（如 Butterworth 滤波器）
// 输入参数:
// x[]: 输入信号向量，长度为 N。
// N: 向量长度（通道数）。
// tau: 滤波器时间常数，用于控制响应速度。
// Ts: 采样时间。
// b[3]: 滤波器分子系数，二阶滤波器的b0, b1, b2
// a[2]: 滤波器分母系数，二阶滤波器的a1,a2
// state[]: 滤波器的内部状态数组，用于存储滤波历史信息。
// out[]: 滤波后的输出向量。
// 功能模块:
// 滤波器初始化（累积前几帧数据，避免依赖单点样本）。
// 滤波操作（通过差分方程更新输出）。
// 状态更新（跟踪内部滤波器状态）。
// state 的作用
// state 是一个状态数组，用于存储滤波器的内部状态信息，包括历史输入和历史输出。它是实现差分方程滤波器不可或缺的一部分。
// 在滤波器计算中，差分方程依赖于之前的输入和输出。state 的作用就是记录这些历史数据，并在每次调用 filterVec 时更新，从而保持滤波器的连续性。
                    ...
                    const double a[2], double state[], vqf_real_t out[])
{
    ...
    if (isnan(state[0])) { // initialization phase
        if (isnan(state[1])) { // first sample
            state[1] = 0; // state[1] is used to store the sample count用来记录已经累积了多少个样本，从而计算这些样本的平均值。其值会递增，直到累积的时间达到滤波器的时间常数（tau），之后滤波器进入正常工作阶段
            for(size_t i = 0; i < N; i++) {
                state[2+i] = 0; // state[2+i] is used to store the sum储存输入数据的累积和
            }
        }
        state[1]++;// 样本数量计数
        for (size_t i = 0; i < N; i++) {
            state[2+i] += x[i];// 样本累积
            out[i] = state[2+i]/state[1];// 计算平均值
        }
        // 判断累积的时间是否已经达到滤波器的时间常数 tau。当达到或超过时间常数时，初始化阶段结束，滤波器进入正常工作状态
        if (state[1]*Ts >= tau) {
            for(size_t i = 0; i < N; i++) {
...
void VQF::setup()
{
    ...
    coeffs.biasV = square(0.1*100.0)*coeffs.accTs/params.biasForgettingTime;//0.1 表示偏置每秒最大漂移0.1∘/s
