...
namespace {
// 这段代码定义了一个模板函数SubscribeWithHandler，用于为ROS节点订阅一个特定的话题，并指定一种消息处理方式。
// 这个模板函数允许用户灵活地为不同类型的消息指定自定义的处理函数，这些处理函数将被自动调用来处理接收到的消息。
// 整个过程依赖于ROS通讯机制（即Publisher-Subscriber模型）和C++的模板与lambda表达式功能。
// Subscribes to the 'topic' using the 'node_handle' and
// calls 'handler' on the 'node' to handle messages. Returns the subscriber.
template <typename MessageType>
::ros::Subscriber SubscribeWithHandler(
    // 这是一个成员函数指针，指向CSMLioWrapper类内部用于处理接收到的消息的函数。
    // 这个函数接收两个参数：一个是话题名（std::string类型），另一个是指向接收消息的智能指针（ConstPtr）。
    void (CSMLioWrapper::*handler)(const std::string&,
        const typename MessageType::ConstPtr&), /*通过函数指针的方式传递回调函数*/
    const std::string& topic,
    ::ros::NodeHandle* const node_handle, //这是一个指向ROS节点句柄的指针，用于调用subscribe函数
    CSMLioWrapper* const node /*这个参数是类对象指针，刚好可以用this指针来传值，这是一个指向CSMLioWrapper类对象的指针，该对象拥有处理函数，可以使用this指针传递*/ ) 
{
    // 函数体中通过调用node_handle->subscribe来订阅给定的话题topic。
    // 这个subscribe函数接收三个参数：话题名、队列大小（这里使用了一个未提及的常量kInfiniteSubscriberQueueSize，
    // 意味着可能是一个无限大小或特定大小的队列）和一个回调函数。在这里，回调函数是通过捕获node和handler参数，
    // 并在lambda表达式中调用handler来构造的
    return node_handle->subscribe<MessageType>(
        ...
        boost::function<void(const typename MessageType::ConstPtr&)>(
            ...
            // lambda表达式[node, handler, topic](const typename MessageType::ConstPtr& msg) {(node->*handler)(topic, msg);}捕获三个变量：node、handler和topic。
            // 当接收到消息时，它调用handler，将topic和消息msg作为参数传递。这实现了在接收到特定类型MessageType的消息时，自动调用指定的处理函数，并传递话题名和消息数
}
    ...
    , motion_compensator_(new ::infinityslam::utils::MotionCompensator)
{
    ...
    // 定时器回调。
    // 这段代码展示了在ROS中如何使用定时器（Timer）来定期执行回调函数，这是常见的ROS编程模式之一，用于定时触发任务。详细解释如下：
    // 设定定时器周期
    //这个变量存储了发布姿态信息的周期（单位：秒）。如果配置项infinityslam_ros::options::pose_publish_period_sec大于0，
    // 则使用该值；否则，默认每0.05秒发布一次。这展示了如何根据配置项灵活地设置定时器的周期。
    double pose_publish_period_sec = 
        infinityslam_ros::options::pose_publish_period_sec > 0 
        ? infinityslam_ros::options::pose_publish_period_sec : 0.05;
    //创建定时器
    //publish_slam_result_timer_：使用node_handle_.createTimer创建了一个定时器，
    // 它将在pose_publish_period_sec指定的周期内重复调用PublishLioResultData成员函数。这个定时器用于定期发布SLAM结果。
...
// 这个ComputeExpectedSensorIds成员函数的作用是，基于给出的配置选项（CSMLioWraPperOptions），
// 来确定哪些传感器ID（涉及特定的话题订阅）对于SLAM系统是必需或可选的，并返回这一集合。
// 该函数体现了如何灵活配置不同SLAM场景下的传感器使用需求。
...
void CSMLioWrapper::PerformPostProcKfLoop()
{
    while (!quit_postproc_flag_) {
        ...
        */
        //等待条件变量: 使用条件变量 cv_ 与 unique_lock 来等待 new_kf_flag_ 标志的改变，即有新的关键帧数据需要处理。
        std::unique_lock<std::mutex> lock(postproc_mutex_);
        cv_.wait(lock, [this](){return new_kf_flag_.load();});
        new_kf_flag_.store(false);
        // LOG(INFO) << "PostProc: triggered. ";
        //检查位姿插值器是否为空: 如果 pose_interpolator_ 为空，直接跳过当前循环。
        if (pose_interpolator_->empty()) {
            LOG(WARNING) << "PostProc: pose interpolator is empty.";
            continue;
        }
        //初始化上次处理的关键帧时间: 如果 last_processed_kf_time_ 还未设置，使用最新时间来初始化它，并跳过当前循环。
        ...
        // 检查是否有新的位姿数据: 如果最后处理时间已经是最新的，继续等待，否则进入数据处理流程。
        ...
        // 收集点云数据: 遍历每个点云数据队列，寻找时间范围内与最新帧重叠的点云数据，并清空过时数据。
        ...
        // 检索静态变换: 清空并重新收集当前所有激光雷达到追踪坐标系的静态变换数据。
        ...
        // 执行后处理
        // 位姿插值: 通过插值器查找指定时间范围内所有的位姿，用于后续的去畸变处理。
