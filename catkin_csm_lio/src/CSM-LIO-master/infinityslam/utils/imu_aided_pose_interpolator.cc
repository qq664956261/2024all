...
// 支持时间段内按时间步长内插查询
// 其目的是在指定的时间段内，按照固定的时间步长对姿态（包括位置和方向）进行插值查询，
// 并将查询结果存储在 output_pose_queue 中。函数允许用户基于IMU数据校正后的插值姿态队列来估计在连续时间间隔内机器人的姿态
bool ImuAidedPoseInterpolator::LookUp(
...
{
    //互斥锁: 使用 UniqLock（应该是 std::unique_lock 的类型别名）对 interp_data_mutex_ 加锁，以保证线程安全地访问姿态插值队列 interpolated_pose_queue_。
    UniqLock lock(interp_data_mutex_);
    ...
    //检查输入时间有效性: 检查输入的 start_time 和 end_time 是否在插值姿态队列的时间范围内，并且 start_time 需小于 end_time。
    if (start_time < interpolated_pose_queue_.front().timestamp ||
        end_time > interpolated_pose_queue_.back().timestamp) {
        return false;
    }
    // 清空输出队列: 清空 output_pose_queue，为新的插值结果做准备。
    ...
    */
    // 遍历和插值: 在给定的起始时间和结束时间之间进行遍历，按照步长 time_step 的间隔递增，
    // 使用时间比例对姿态进行插值。插值使用线性插值对位置进行计算，使用球面线性插值（slerp）对四元数表示的旋转进行计算。
