    ...
    namespace sensor
    {
        ...
        namespace
        {
                                        ...
                                        const float max_range)
            {
                return point_cloud.copy_if([max_range](const RangefinderPoint &point)
                                           { return point.position.head(2).norm() <= max_range; });
            }
            ...
            {
                if (point_cloud.size() <= 105/*options.min_num_points()*/)
                {
                    // 'point_cloud' is already sparse enough.
                    return point_cloud;
                }
                PointCloud result = VoxelFilter(point_cloud, 0.5/*options.max_length()*/);

                if (result.size() >= 105 /*options.min_num_points()*/)
                {
                ...
                for (float high_length = 0.5/*options.max_length()*/;
                     high_length > 1e-2f * 0.5/*options.max_length()*/; high_length /= 2.f)
                {
                    ...
                    if (result.size() >= 105/*options.min_num_points()*/)
                    {
                        ...
                        while ((high_length - low_length) / low_length > 1e-1f)
                        {
                            ...
                            if (candidate.size() >= 105/*options.min_num_points()*/)
                            {
            ...
            /*,const proto::AdaptiveVoxelFilterOptions &options*/)
        {
            return AdaptivelyVoxelFiltered(
                /*options, */FilterByMaxRange(point_cloud, 50/*options.max_range()*/));
        }
