...
void FastGICP<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  Eigen::Isometry3d x0 = Eigen::Isometry3d(guess.template cast<double>());           //x0: 4X4   初始值为单位矩阵
  ...
  for(int i = 0; i < max_iterations_ && !converged_; i++) {
    ...
    update_correspondences(x0);                    //找对应点
...
void FastGICP<PointSource, PointTarget>::update_mahalanobis(const Eigen::Isometry3d& trans) {
  ...
  mahalanobis.resize(input_->size());              //  std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>> mahalanobis;  马氏距离
...
#pragma omp parallel for num_threads(num_threads_)
  for(int i = 0; i < cloud->size(); i++) {
    ...
    Eigen::Matrix4d cov = neighbors * neighbors.transpose() / k_correspondences_;                //每个点临近点的协方差
    ...
    if(regularization_method_ == RegularizationMethod::NONE) {
      ...
      switch(regularization_method_) {
        ...
        case RegularizationMethod::NORMALIZED_MIN_EIG:
          values = svd.singularValues() / svd.singularValues().maxCoeff();   //maxCoeff()求最大值
          ...
          values = values.array().max(1e-3);                                                                //返回与1e-3比较大的值
          break;
      }
      covariances[i].setZero();
